...
import { GetVisitorPhotoService } from "../../services/visitor/IdVisitorPhoto.service";

class ViewVisitorPhotoController {
  async handle(req: Request, res: Response) {
    try {
      const { id } = req.params;

      const getVisitorPhotoService = new GetVisitorPhotoService();
      const photo = await getVisitorPhotoService.execute(id);
...
export { ViewVisitorPhotoController };